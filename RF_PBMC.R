...
process_data <- function(input.data){
  ...
  #TODO min.cells, min.genes as optional arguments
  seuratObj <- CreateSeuratObject(raw.data = input.data, min.cells = 3, 
                                  min.genes = 200, project = "10X_data")
  #TODO use logging instead of print statements
  print(seuratObj)
  
  #TODO break out filtering steps to separate function (ie thru FilterCells)
  ...
  #TODO parameterize low.thresholds, high.thresholds
  seuratObj <- FilterCells(object = seuratObj, subset.names = c("nGene", 
                            "percent.mito"), low.thresholds = c(200, -Inf),
                            high.thresholds = c(2500, 0.05))
  seuratObj <- NormalizeData(object = seuratObj, normalization.method = 
  ...
  #TODO for default PCAPlot, each PC should be in at least one plot
  # eg 1v2, 2v3, 3v4, 4v5
  PrintPCA(object = seuratObj, pcs.print = 1:5, genes.print = 5, 
           use.full = FALSE)
  ...
  #TODO make JackStraw PCs and optional argument 
  #TODO parameterize num.replicate for JackStraw
